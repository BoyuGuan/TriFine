...
logger = logging.getLogger('trainVMTTagText')
...
def train(model, trainDataset, validDataset, tokenizer, args):
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks= [EarlyStoppingCallback(
            early_stopping_patience=10,               # 在连续5次评估的eval_loss不再下降时停止训练
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tag_language", type=str, default='zh', help='Target language (zh or en)')
    ...
    parser.add_argument("-t", "--tag", type=str, default="location")
    ...
    if args.train_dataset_path is None:
        args.train_dataset_path = f'./data/{args.source_language}_{args.tag_language}_emotion_tagged_Clips.json'
    if args.output_dir is None:
        args.output_dir = f'./checkpoint/model/tag_text-{args.tag}-{args.source_language}-{args.target_language}-{datetime.now().strftime("%Y-%m-%d-%H-%M")}'
    ...
    allDataset = vmtTagTextDataset(args.train_dataset_path, tokenizer,args.tag, args.source_language, args.target_language, args.max_length, 65000)
    logger.info(f'\nDataset size of tag {args.tag} -- vmtTagTextDataset: {len(allDataset)}\n')
    ...
    if args.test_dataset_path is None:
        ...
        validDataset = vmtTagTextDataset(args.test_dataset_path, tokenizer,args.tag, args.source_language, args.target_language, args.max_length, 65000)
